...
import {
  ...
  checkoutSchema,
  connectionActivateSchema,
  tenantChargeSchema,
  tenantDocumentsSchema,
  tenantInviteSchema,
  tenantSchema
...
router.post('/connections/activate', authenticate, requireRole(['owner', 'admin']), validate(connectionActivateSchema), activateConnection);
...
router.post('/:id/checkout', authenticate, requireRole(['owner', 'admin']), validate(checkoutSchema), checkoutTenant);
router.post('/:id/documents', authenticate, requireRole(['owner', 'admin']), validate(tenantDocumentsSchema), uploadDocuments);

// Additional charges & manual billing endpoints
router.post('/:id/charges', authenticate, requireRole(['owner', 'admin']), validate(tenantChargeSchema), addTenantCharge);
